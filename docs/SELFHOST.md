...
**下一步**：M6 把 boot 里 AST dump / cps 也迁走；拆 `emit_insn`；不要把大块逻辑塞进 `emit_insn`。
...
**仍待**：
1. **M6 L6**：compiler/qemu 只经原生 `yc_A`；boot 的 lex/elf/emit/encode 单元用 `yc_A` 编跑。parse/anf/`print` 列表、cps/callcc/float/bignum 仍走 C。`yc_A`/`yc_B` iso 仍在。
