...
- 已落地（按类）：`text`（`str` / `bytes` / `fmt` / `json`）、`data`（`list` / `map` / `fmap` / `hash`）、`num`（`math`）、`fs`（`io` / `path`）、`os`（`env` / `cli` / `log`）、`net`（`tcp` / `http`，`import net.tcp.http` 会链 `net.tcp`）、`gui`（`yui`）、`native`（`ffi`）、`tools`（`compiler` / `profile`）、`test`（`assert`）。`math`：整数 gcd/pow/isqrt；f64 用牛顿/泰勒，不链 libm。`json` tagged：`["N",n]` / `["S",s]` / `["A",xs]` / `["O",pairs]` / `["T"]` `["F"]` `["Z"]`。
