...
| 常见库 | 分类短名（`text` / `data` / `num` / `fs` / `os` / `net` / `gui` / `native` / `yc` / `test`） | 仓库根 `pkg/<类>/<名>.yac` | `import fs.path`；项目自己的库也放 `./pkg` |
...
- 已落地（按类）：`text`（`str` / `bytes` / `fmt` / `json`）、`data`（`list` / `map` / `fmap` / `hash`）、`num`（`math`）、`fs`（`io` / `path`）、`os`（`env` / `cli` / `log`）、`net`（`tcp` / `http`，`import net.http` 会链 `net.tcp`）、`gui`（`yui`）、`native`（`ffi`）、`yc`（`compiler` / `profile`）、`test`（`assert`）、`lang`（`js`：JS 子集 `js_to_yac` 转 yac 源码）。`math`：整数 gcd/pow/isqrt；f64 用牛顿/泰勒，不链 libm。`json` tagged：`["N",n]` / `["S",s]` / `["A",xs]` / `["O",pairs]` / `["T"]` `["F"]` `["Z"]`。
