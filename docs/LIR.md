...
> 自编译**（`yc_l1` 成功编出 `yc_l1b`）。但**第二代自举 `yc_l1b` 坏掉**
> （`error: bad arguments`），原因未定，于是这次删改**无法验证**。能验证之前，保留原样。
>
...
3. 但**二代自举 `yc_l1b` 坏掉**（`error: bad arguments`），原因未定 → 这次删改
   **无法验证**。
...
| **4** | `$` 不对称 → **决定保留不修**（§9.6） · 清死 handler（§9.5）**部分完成**：无争议的 4 条（`sal` / `sar` / `$lea` / `gcall`）已删，27 条内联原语待第 1 步决策 | 规范立起来后清死代码才有依据 |
