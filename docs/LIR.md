...
> **`print` 不在这一族。** 它在 `emit` 里**连分支都没有** —— 是**幽灵**，不是死
> handler（§9.5）。旧文档把它列在这里是错的。源级 `print e` 在 parser 里就降成了
> `call print(e, true)`。
